    ...
    try {
        ...
        ws.onmessage = async (event) => {
            ...
            if (data.type === 'ready') {
                ...
                // 도구 사용 알림
                addToolMessage('using', data.toolName, data.input);
                updateStatus('active', `🔧 도구 사용 중: ${data.toolName}...`);
            } else if (data.type === 'tool-result') {
                // 도구 결과 표시
                addToolMessage('result', data.toolName, data.result);
                updateStatus('active', '🤖 AI가 응답 준비 중...');
...
function addToolMessage(type, toolName, data) {
    ...
    if (type === 'using') {
        // 도구 사용 중
        const toolIcon = toolName === 'get_weather' ? '🌤️' : '🔍';
        messageDiv.innerHTML = `
            <div class="tool-header">
                <span class="tool-icon">${toolIcon}</span>
                <span class="tool-name">${getToolDisplayName(toolName)}</span>
            </div>
            <div class="tool-params">${formatToolParams(data)}</div>
        `;
    } else if (type === 'result') {
        // 도구 결과
        const toolIcon = toolName === 'get_weather' ? '🌤️' : '🔍';
        messageDiv.innerHTML = `
            <div class="tool-header">
                <span class="tool-icon">${toolIcon}</span>
                <span class="tool-name">${getToolDisplayName(toolName)} 결과</span>
            </div>
            <div class="tool-result">${formatToolResult(toolName, data)}</div>
        `;
    }
...
function formatToolParams(params) {
    if (params.city) return `도시: ${params.city}`;
    if (params.query) return `검색어: ${params.query}`;
...
function formatToolResult(toolName, result) {
    ...
    if (toolName === 'get_weather') {
        return `
            <div class="weather-result">
                ...
                <div>🌡️ ${result.temperature}</div>
                <div>☁️ ${result.condition}</div>
    ...
    if (toolName === 'web_search') {
        ...
        result.results.forEach(item => {
            if (item.type === 'answer') {
                html += `<div class="search-answer"><strong>${item.title}:</strong> ${item.text}</div>`;
            } else if (item.type === 'related') {
                html += `<div class="search-related">• ${item.text}</div>`;
            }
        });
        html += `</div>`;
        return html;
