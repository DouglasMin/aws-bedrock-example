...
class AgentsForAmazonBedrock:
    ...
    def __init__(self):
        ...
        # Get region from environment or boto session
        self._region = os.environ.get("AWS_REGION") or self._boto_session.region_name
        if not self._region:
            self._region = "us-east-1"  # Default fallback
        ...
        self._bedrock_agent_runtime_client = boto3.client(
            "bedrock-agent-runtime", region_name=self._region, config=long_invoke_time_config
        )
